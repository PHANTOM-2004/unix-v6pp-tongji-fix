...
int ProcessManager::NewProc() {
  ...
  //                    (unsigned
  //                    long)u.u_MemoryDescriptor.m_UserPageTableArray,
  //                    sizeof(PageTable) *
...
int ProcessManager::Swtch() {
   ...
   * 单从进程切换的角度，完全可以由下台进程挑选就绪进程上台。
   * 但是，考虑时钟中断。 一秒末的
   * 例行处理，最好系统idle时，其次是在执行应用程序过程中；不可以放在内核执行过程中。
   * 如何判断？
...
loop:
  ...
  for (int i = 0; i < ProcessManager::NPROC; i++) {
    ...
    bool pFlagIsSLOAD =
        (this->process[i].p_flag &
         (int(Process::SSYS) | int(Process::SLOCK) | int(Process::SLOAD))) ==
        int(Process::SLOAD);

    bool statIsSWAITOrSSTOP = (this->process[i].p_stat == Process::SWAIT ||
                               this->process[i].p_stat == Process::SSTOP);
  ...
  for (int i = 0; i < ProcessManager::NPROC; i++) {
    ...
    bool pFlagIsSLOAD =
        (this->process[i].p_flag &
         (int(Process::SSYS) | int(Process::SLOCK) | int(Process::SLOAD))) ==
        int(Process::SLOAD);
    bool pStatIsSWAITOrSSTOP = this->process[i].p_stat == Process::SWAIT ||
                               this->process[i].p_stat == Process::SSTOP;
...
found2:
  ...
  if (pSelected->p_textp != NULL) {
    ...
    if (pText->x_ccount == 0) {
       ...
       */
      if (bufMgr.Swap(pText->x_daddr, desAddress, pText->x_size, Buf::B_READ) ==
          false) {
        goto err;
      }
      /* 共享正文段在内存中的起始地址 */
  ...
  if (bufMgr.Swap(pSelected->p_addr /* blkno */, desAddress, pSelected->p_size,
                  Buf::B_READ) == false) {
    goto err;
  }
  Kernel::Instance().GetSwapperManager().FreeSwap(
      pSelected->p_size, pSelected->p_addr /* blkno */);
  ...
  while (true) {
    for (i = 0; i < NPROC; i++) {
      if (u.u_procp->p_pid == process[i].p_ppid) {
        Diagnose::Write("Process %d (Status:%d)  ", process[i].p_pid,
                        process[i].p_stat);
        hasChild = true;
        /* 睡眠等待直至子进程结束 */
...
void ProcessManager::Exec() {
  ...
  if (fileMgr.Access(pInode, Inode::IEXEC) ||
      (pInode->i_mode & Inode::IFMT) != 0) {
  ...
  if (textSize + dataSize + stackSize + PageManager::PAGE_SIZE >
      MemoryDescriptor::USER_SPACE_SIZE - textAddr) {
    fileMgr.m_InodeTable->IPut(pInode);
  ...
  for (int i = 0; i < ProcessManager::NTEXT; i++) {
    if (NULL == this->text[i].x_iptr) /* 记下找到的第一个空闲text结构 */
    {
      if (NULL == pText) {
        ...
        pInode ==
        this->text[i]
            .x_iptr) /* 如果，这不是一个空闲text结构，看一下text结构指向的可执行文件是exec系统调用要执行的应用程序吗？
                      */
    {
      this->text[i].x_count++;
      ...
      pText =
          NULL; /* 与其它进程共享同一正文段，则pText重新清零，否则指向一空闲Text结构
                 */
      break;
    }
  }
  ...
  if (NULL != pText) {
     ...
     * 则会错误地判断：pInode (当前exe对应Inode) ==
     * this->text[i].x_iptr(之前exe文件Inode)，
  ...
  unsigned int newSize = ProcessManager::USIZE +
                         u.u_MemoryDescriptor.m_DataSize +
  ...
  u.u_MemoryDescriptor.EstablishUserPageTable(textAddr, textSize, dataAddr,
                                              dataSize, stackSize);
  ...
  // Utility::MemCopy(fakeStack | 0xC0000000, MemoryDescriptor::USER_SPACE_SIZE
  // - parser.StackSize, parser.StackSize);
  Utility::MemCopy((fakeStack + (unsigned long)(allocLength)-stackSize) |
                       0xC0000000,
                   MemoryDescriptor::USER_SPACE_SIZE - stackSize, stackSize);
  ...
  //	for (unsigned int i = 0; i < (unsigned long)ExecShell - (unsigned
  // long)runtime; i++)
  //	{
  ...
  for (int i = User::EAX - 4; i < User::EAX - 4 * 7; i = i - 4) {
    u.u_ar0[i] = 0; /* 下标写成  User::EAX + i
                       可读性要强一些，但是运算速度慢了。就小抠，追求速度吧 */
  }

  /* 将exe程序的入口地址放入核心栈现场保护区中的EAX作为系统调用返回值，这个是runtime要用
   */
  u.u_ar0[User::EAX] =
      isPE ? peParser.EntryPointAddress : elfParser.entryPointAddr;

  /* 构造出Exec()系统调用的退出环境，使之退出到ring3时，开始执行user code */
  struct pt_context *pContext = (struct pt_context *)u.u_arg[4];
  pContext->eip =
      0x00000000; /* 退出到ring3特权级下从线性地址0x00000000处runtime()开始执行
                   */
...
Process *ProcessManager::Select() {
  ...
  while (true) {
    ...
    for (int count = 0; count < NPROC; count++) {
      ...
      if (Process::SRUN == process[i].p_stat &&
          (process[i].p_flag & Process::SLOAD) != 0) {
...
void ProcessManager::XSwap(Process *pProcess, bool bFreeMemory, int size) {
  ...
  /* blkno记录分配到的交换区起始扇区号 */
  int blkno =
      Kernel::Instance().GetSwapperManager().AllocSwap(pProcess->p_size);
  if (0 == blkno) {
  ...
  if (false == Kernel::Instance().GetBufferManager().Swap(
                   blkno, pProcess->p_addr, size, Buf::B_WRITE)) {
