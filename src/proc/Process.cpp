...
void Process::Exit() {
  ...
  OpenFileTable &fileTable =
      *Kernel::Instance().GetFileManager().m_OpenFileTable;
  ...
  Utility::DWordCopy((int *)&u, (int *)pBuf->b_addr,
                     BufferManager::BUFFER_SIZE / sizeof(int));
  bufMgr.Bwrite(pBuf);
  ...
  for (i = 0; i < ProcessManager::NPROC; i++) {
    if (current->p_pid == procMgr.process[i].p_ppid) {
      Diagnose::Write("My:%d 's child %d passed to 1#process", current->p_pid,
                      procMgr.process[i].p_pid);
...
void Process::SStack() {
  ...
  if (!u.u_MemoryDescriptor.VirtualAddressCheck(md.m_TextStartAddress,
                                                md.m_TextSize, md.m_DataSize,
                                                md.m_StackSize)) {
    return;
  }

  // if (false == u.u_MemoryDescriptor.EstablishUserPageTable(
  //                  md.m_TextStartAddress, md.m_TextSize,
  //                  md.m_DataStartAddress, md.m_DataSize, md.m_StackSize)) {
...
void Process::SBreak() {
  ...
  if (!u.u_MemoryDescriptor.VirtualAddressCheck(
          md.m_TextStartAddress, md.m_TextSize, newSize, md.m_StackSize)) {
    return;
  }

  // if (false == u.u_MemoryDescriptor.EstablishUserPageTable(
  //                  md.m_TextStartAddress, md.m_TextSize,
  //                  md.m_DataStartAddress, newSize, md.m_StackSize)) {
  //   // 系统调用出错时，不可以用这种方式返回。执行这条路径会导致 u.u_intflg ==
  //   // 1，u.u_error被错误修改为EINTR（4）；无论何故导致系统调用失败。
  //   // aRetU(u.u_qsav);
  //   return;
  // }
...
void Process::Ssig() {
  ...
  /* 这几个信号不许设置 */
  if (signalIndex <= 0 || signalIndex >= User::NSIG ||
      signalIndex == User::SIGKILL) {
