...
public:
  ...
  void MapStackEntrys(unsigned long stackSize,
                      unsigned long stackPageIdxInPhyMemory);

  /* @comment
   * 原unixv6中sureg()函数.原函数用于将进程u区中的uisa和uisd两数组中的内存页映射数据映射到UISA与UISD
  ...
  /*
   * @comment 原unix
   * v6中estabur()函数，用于建立用户态地址空间的相对地址映射表，然后调用
  ...
  bool EstablishUserPageTable(unsigned long textVirtualAddress,
                              ...
                              unsigned long dataSize, unsigned long stackSize);
  // @comment
  // 用于检查是否允许建立UserPageTable
  bool VirtualAddressCheck(unsigned long textVirtualAddress,
                           unsigned long textSize, unsigned long dataSize,
...
private:
   ...
   * unsigned int size:
   * 需要映射的虚拟地址大小(以字节为单位) unsigned long phyPageIdx:
   * 其实物理页索引号(页为单位) bool isReadWrite:
   * 页属性，true为可读可写页
   */
