...
void SystemCall::SystemCallEntrance() {
   ...
   * else  {  //中断前==核心态
   * 		改先前态为用户态；
   * 执行设备(陷入)处理子程序；恢复现场，退出至前一层中断；}
   *
   * 由于体系结构的不同(x86的PSW不记录先前态)，x86上通过判断核心栈CS中OLD_CPL ==
   * 0x3； 判断先前是用户态或者核心态。
   *
  ...
  RestoreContext(); // SysCallRestore();	/*
                    // 此后EAX中存放系统调用返回值，防止一切可能的修改 */
...
void SystemCall::Trap(struct pt_regs *regs, struct pt_context *context) {
  ...
  /* 注: Unix
   * V6++将系统调用出错结果返回给用户程序的方式和V6(通过PSW中的EBIT)有所区别!
...
/*	21 = mount	count = 3	*/
int SystemCall::Sys_Smount() { return 0; /* GCC likes it ! */ }

/*	22 = umount  count = 1	*/
int SystemCall::Sys_Sumount() { return 0; /* GCC likes it ! */ }
...
/*	26 = ptrace	count = 3	*/
int SystemCall::Sys_Ptrace() { return 0; /* GCC likes it ! */ }
...
int SystemCall::Sys_Sslep() {
  ...
  /*
   * 对   if ( Time::tout <= Time::time || Time::tout > wakeTime )
   * 中判断条件的解释： 1、系统先前设置的所有闹钟均已到期。
   * 其后，第一个设置闹钟的进程看到的是条件 tout <=
   * time成立，将自己的waketime写入tout变量。
   * 2、系统中，存在闹钟未到期的进程。如果有进程设置闹钟，看到的是条件tout >
   * time，进程比对tout变量和自己的waketime，令tout变量的值是所有进程waketime的最小值。
   *
   * 原先的注释：
   ...
   *
   * 原先的注释不对。如果while循环的判断条件是'wakeTime >=
   * Time::time'，执行sleep(0)的进程将把waketime和tout设为上个整数秒。整数秒时钟中断处理程序会time++，之后
   * 1、如果不再有进程设置新闹钟，系统的闹钟服务就瘫痪了。这是因为，
   * time==tout的条件永远无法满足，时钟中断处理程序不再会唤醒任何因设置了闹钟而入睡的进程。
...
/*	44 = prof	count = 4	*/
int SystemCall::Sys_Profil() { return 0; /* GCC likes it ! */ }
...
int SystemCall::Sys_Getppid() {
  ...
  int const cur_pid = u.u_arg[0];
  u.u_ar0[User::EAX] = -1;
  for (int i = 0; i < ProcessManager::NPROC; i++) {
    if (procMgr.process[i].p_pid == cur_pid) {
